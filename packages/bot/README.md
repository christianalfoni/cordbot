# Cordbot - Discord Bot CLI with Claude Code SDK
...
- **🌍 Global CLI Tool**: Install once with `npm install -g cordbot`, use anywhere
- **📁 Directory-Based**: Each project directory has its own configuration and synced channels
- **🤖 Autonomous Agent**: Full Claude Code SDK capabilities with dangerous mode enabled
- **💬 Thread-Based Sessions**: Each Discord thread maintains persistent Claude sessions
- **📋 Plan Attachments**: Automatically sends implementation plans as downloadable `.md` files
- **⏰ Scheduled Tasks**: Configure autonomous tasks with `.claude-cron` files
- **🔄 Hot Reload**: Watches for cron file changes and reloads automatically
- **🎨 Interactive Setup**: First-time setup wizard with validation

## Installation

```bash
# Install globally
npm install -g cordbot
...
Before running Cordbot, you need:

1. **Discord Bot Token** - Create a bot at https://discord.com/developers/applications
2. **Discord Guild (Server) ID** - Right-click your server → Copy Server ID
...
On first run, Cordbot will prompt you for configuration:
...
? Would you like to configure Cordbot now? (Y/n)
...
After running Cordbot, your project will look like this:

```
my-project/
├── .env                      # Configuration (git-ignored)
├── .claude/                  # Bot state (git-ignored)
│   ├── config.json          # Discord bot config
│   ├── mappings.db          # SQLite: Discord ID ↔ Session ID
│   └── sessions/            # Agent session persistence
├── CLAUDE.md                # Root bot instructions
└── (Discord channel folders)/
    ├── general/
    │   ├── CLAUDE.md        # Channel-specific context
...
👤 You: Hey Claude, what's the latest commit?

[Bot creates thread: "You: Hey Claude, what's the latest..."]

🤖 Claude: The latest commit is "fix auth flow" (abc123)
          from 2 hours ago.
```
...
```
[In thread]
👤 You: Can you explain that change?

🤖 Claude: That commit fixed the JWT validation logic...

👤 You: Can you run the tests?

⚙️ bash: npm test

🤖 Claude: All tests passed! ✓ 23 tests
```
...
git clone https://github.com/yourusername/cordbot.git
cd cordbot
...
**Important**: Cordbot runs in "dangerous mode" with full system access:
