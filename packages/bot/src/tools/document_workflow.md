...
### 3. Preview with Shareable Links

Let users preview your changes before converting back:

**Tool:** `create_share_link`
```
Create a shareable link for document.md so the user can preview it
```

This generates a temporary URL (expires in 1 hour) where users can view the current markdown in their browser.

**Benefits:**
- Quick preview without downloading
- See formatting as it will appear
- Verify changes before final conversion
...
3. Create preview link: `create_share_link Brand_Guide.md`
4. Show user: "I've updated the document. Preview here: [link]"
...
### Document Tools
- **create_share_link** - Generate temporary preview URLs (1 hour expiration)
...
Better: "I've updated the document. Preview: [link]. If approved, I'll generate the Word version."
```
...
- Use share links for frequent previews
...
3. Create share link for preview
4. User reviews → requests more changes
5. Edit again → new share link
...
- `.md` - Shareable preview links via `create_share_link`
...
Generated by `create_share_link`:
- **Expiration:** 1 hour (extended on each access)
- **Access:** Anyone with the link
- **Scope:** Files in the current channel's workspace only
- **Format:** `https://your-bot.fly.dev/share/[token]`
...
# To preview changes
create_share_link <filename.md>
→ Returns: https://bot.com/share/abc123
→ Expires: 1 hour (extended on access)
...
2. **Share links are free** - Use them generously for previews
